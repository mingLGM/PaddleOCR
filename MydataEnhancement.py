import os
import cv2
import numpy as np
import json
import argparse
from PIL import Image
from MyOcr2paddleocr import split_dataset_labels

def letterbox_image(img, intermediate_size=640, target_size=(1280, 1280), color=(114, 114, 114)):
    """
    PIL版本的LetterBox：将图像缩放后填充到目标尺寸（兼容中文路径的图像处理）
    
    Args:
        img: PIL Image对象（需为RGB模式）
        intermediate_size: 中间缩放尺寸（长边目标值）
        target_size: 最终目标尺寸（宽, 高）
        color: 填充颜色（R, G, B）
    
    Returns:
        padded_img: 处理后的PIL图像
        ratio: 缩放比例（w_ratio, h_ratio）
        pad: 填充量（top, bottom, left, right）（与原CV2逻辑一致，用于坐标转换）
    """
    w, h = img.size  # PIL的size是（宽，高）
    target_w, target_h = target_size
    
    # 1. 计算缩放比例（长边缩放到intermediate_size）
    scale = intermediate_size / max(w, h)
    new_w = int(w * scale)
    new_h = int(h * scale)
    
    # 2. 缩放图像（LANCZOS重采样=CV2的INTER_LINEAR）
    resized_img = img.resize((new_w, new_h), Image.Resampling.LANCZOS)
    
    # 3. 创建目标画布并填充颜色
    padded_img = Image.new('RGB', (target_w, target_h), color=color)
    
    # 4. 居中粘贴缩放后的图像
    left_pad = (target_w - new_w) // 2
    top_pad = (target_h - new_h) // 2
    padded_img.paste(resized_img, (left_pad, top_pad))
    
    # 计算填充量（与原CV2逻辑一致，用于后续标注坐标转换）
    dw = target_w - new_w
    dh = target_h - new_h
    top = dh // 2
    bottom = dh - top
    left = dw // 2
    right = dw - left
    
    return padded_img, (scale, scale), (top, bottom, left, right)

def update_annotation_points(points, ratio, pad, target_size):
    """
    更新标注点的坐标以匹配letterbox处理后的图像，并确保坐标不越界。
    
    Args:
        points: 原始标注点列表 [[x1, y1], [x2, y2], ...].
        ratio: 缩放比例 (w_ratio, h_ratio).
        pad: 填充量 (top, bottom, left, right).
        target_size: 目标图像尺寸 (宽, 高).
        
    Returns:
        new_points: 更新后的点坐标，确保在图像范围内.
    """
    top, bottom, left, right = pad
    w_ratio, h_ratio = ratio
    target_w, target_h = target_size
    
    new_points = []
    for point in points:
        # 应用缩放
        x = point[0] * w_ratio
        y = point[1] * h_ratio
        # 应用填充偏移
        x += left
        y += top
        
        # 确保坐标不超出图像边界[4,8](@ref)
        x = max(0, min(x, target_w - 1))  # x在0到target_w-1之间
        y = max(0, min(y, target_h - 1))  # y在0到target_h-1之间
        
        new_points.append([int(round(x)), int(round(y))])
    
    return new_points

def process_dataset(image_dir, label_file_path, output_image_dir, output_label_path, 
                   intermediate_size=640, target_size=(640, 640)):
    """
    处理目标检测数据集：使用PIL读取图像并进行LetterBox缩放填充，同步更新标注点坐标，最终保存处理后的图像与标注文件。
    Args:
        image_dir (str): 原始图像的存储目录（支持中文路径）；
        label_file_path (str): 原始标注文件路径，每行格式为「图像相对路径\tJSON标注字符串」（需UTF-8编码）；
        output_image_dir (str): 处理后图像的保存目录（自动创建不存在的父目录）；
        output_label_path (str): 更新后标注文件的保存路径（UTF-8编码，保留中文）；
        intermediate_size (int, optional): LetterBox的中间缩放尺寸——图像**长边**会被缩放至该值（默认640）；
        target_size (tuple, optional): LetterBox的最终目标尺寸（格式：`(宽, 高)`）——缩放后的图像会填充至此尺寸（默认(640, 640)）。
    Returns:
        None: 结果直接保存至指定输出路径，无返回值。
    """
    # 创建输出目录
    os.makedirs(output_image_dir, exist_ok=True)
    
    # 读取原始标注
    with open(label_file_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    
    new_lines = []
    
    for line in lines:
        parts = line.strip().split('\t')
        if len(parts) < 2:
            continue
            
        img_path = parts[0]
        annotation_str = parts[1]
        full_img_path = os.path.join(image_dir, img_path)
        
        # 检查原图像是否存在
        if not os.path.exists(full_img_path):
            print(f"警告：图像不存在 → {full_img_path}，跳过")
            continue
        
        try:
            img = Image.open(full_img_path)
            if img.mode != 'RGB':  # 强制转为RGB（避免RGBA等模式报错）
                img = img.convert('RGB')
        except Exception as e:
            print(f"警告：无法读取图像 → {full_img_path}，错误：{e}，跳过")
            continue
        
        processed_img, ratio, pad = letterbox_image(img, intermediate_size, target_size=target_size)
        
        output_img_path = os.path.join(output_image_dir, img_path)
        try:
            # 创建父目录（解决子目录不存在的问题）
            os.makedirs(os.path.dirname(output_img_path), exist_ok=True)
            processed_img.save(output_img_path)  # PIL保存图像
        except Exception as e:
            print(f"警告：无法保存图像 → {output_img_path}，错误：{e}，跳过")
            continue
        
        try:
            annotations = json.loads(annotation_str)
            updated_annotations = []
            for ann in annotations:
                if 'points' in ann:
                    updated_points = update_annotation_points(
                        ann['points'], ratio, pad, target_size
                    )
                    updated_ann = ann.copy()
                    updated_ann['points'] = updated_points
                    updated_annotations.append(updated_ann)
                else:
                    updated_annotations.append(ann)
            # 构建新标注行
            new_line = f"{img_path}\t{json.dumps(updated_annotations, ensure_ascii=False)}\n"
            new_lines.append(new_line)
        except json.JSONDecodeError:
            print(f"警告：标注解析失败 → {annotation_str}，跳过")
            continue
    
    # 写入更新后的标注文件
    with open(output_label_path, 'w', encoding='utf-8') as f:
        f.writelines(new_lines)
    
    print(f"处理完成！共处理 {len(new_lines)} 个样本")
    print(f"输出图像目录 → {output_image_dir}")
    print(f"输出标注文件 → {output_label_path}")

if __name__ == "__main__":
    # 使用LetterBox处理数据集图像和标注
    image_dir        = r"/home/hc/work/lzm/datasets/gangban/paddleocr_dataset/det"          # 原始图像目录
    label_file       = r"/home/hc/work/lzm/datasets/gangban/paddleocr_dataset/det/det_label.txt"      # 原始标注文件
    output_image_dir = r"/home/hc/work/lzm/datasets/gangban/paddleocr_dataset/det_640"     # 输出图像目录
    output_label_file= r"/home/hc/work/lzm/datasets/gangban/paddleocr_dataset/det_640/det_label_640.txt" # 输出标注文件
    intermediate_size = 640          # 原图先按比例缩放到此中间尺寸
    target_size      = (640, 640)     # 最终填充到目标尺寸 (宽, 高)
    
    process_dataset(image_dir, label_file, output_image_dir, output_label_file, intermediate_size, target_size)
    split_dataset_labels(output_label_file, split_ratio=0.9)